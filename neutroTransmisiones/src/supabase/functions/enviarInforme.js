...
serve(async (req) => {
  ...
  try {
    ...
    const transporter = createTransport({
      host: "mail.NeutroTransmisiones.cl",
      port: 465,
      secure: true,
      auth: {
    ...
    const cuerpoHtml = `
      <div style="font-family: Arial, sans-serif; color: #333;">
        ...
        <hr>
        <p style="font-size: 12px; color: #666;">NeutroTransmisiones - Servicios Mecánicos</p>
      </div>
    `;

    const info = await transporter.sendMail({
      from: `"NeutroTransmisiones Soporte" <${Deno.env.get("SMTP_USER")}>`,
      to: emailCliente,
      subject: asunto,
      html: cuerpoHtml,
    });
