...
<template>
    <tr class="hover:opacity-80 transition-colors cursor-pointer" @click="irADetalle(orden.id)">
        ...
        <td class="p-4 text-center whitespace-nowrap">
            <span class="neutro-font">{{ formatearFecha(orden.fecha_ingreso) }}</span>
        </td>
        <td class="p-4 text-center">
            <span class="px-3 py-1 rounded-full text-xs font-medium"
                :style="{ backgroundColor: estado.color}"
                :class="!estado.texto ? 'neutro-font' : 'text-white'">
                {{ estado.estado }}
            </span>
        </td>
        <td class="p-4 text-center" @click.stop>
            <span v-if="orden.id_empleado" class="neutro-font">{{ orden.trabajadores?.nombre }}</span>
            <button v-if="!empleadoAsignado" @click="abrirModalAsignar" class="mt-2 flex rounded-lg neutro-font neutro-font justify-center items-center px-3 py-2 w-full mx-auto transition-colors hover:bg-yellow-400 cursor-pointer">
                <span class="font-bold neutro-font text-sm mr-2">Asignar Técnico</span>
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
        ...
        <td class="p-4 text-center neutro-font">
    ...
    <div class="neutro-background neutro-font rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
      ...
      <div class="p-6">
        ...
        <select v-model="empleadoSeleccionado" class="w-full rounded-lg px-3 py-2.5 neutro-font servi-dark-font font-bold">
      ...
      <div class="px-6 py-4 flex justify-end gap-3">
        <button @click="modalAsignar = false" class="px-4 py-2 text-sm font-medium neutro-font rounded-lg cursor-pointer">
          Cancelar
        </button>
        <button 
          @click="asignarOrden" 
          :disabled="!empleadoSeleccionado || cargando"
          class="px-4 py-2 text-s neutro-font neutro-font rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 cursor-pointer"
        >
