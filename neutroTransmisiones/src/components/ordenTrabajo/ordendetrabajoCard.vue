...
<template>
  <div :class="estado.estado + ' border-t-10'" :style="{ borderTopColor: estado.color }" class="neutro-primary neutro-font mx-2 card-shadow p-3 mb-3 flex flex-col gap-2 overflow-hidden transition-all hover:shadow-md">
    ...
    <div class="flex flex-col gap-1 text-sm">
      ...
      <div class="info-row" v-if="orden.fecha_ingreso">
        <span class="label neutro-font">Fecha Ingreso: </span>
        <span class="valor neutro-font"> {{ formatearFecha(orden.fecha_ingreso) }}</span>
      </div>
    </div>

    <div class="flex mt-2 pt-2 border-t border-gray-100 items-center justify-between">
      
      <RouterLink :to="{ name: 'ver-orden-de-trabajo', params: { id: orden.id } }" class="neutro-font neutro-font p-2 rounded-lg w-full text-center transition-transform hover:scale-110 shadow-sm">
        <span class="neutro-font text-sm">Ver Detalles</span>
      </RouterLink>
    </div>
  ...
  <div v-if="modalAsignar" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="neutro-primary neutro-font rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in duration-200">
      
      <div class="px-6 py-4">
        <h2 class="text-lg font-bold neutro-font">Asignar Responsable</h2>
        <p class="text-sm neutro-font">OT #{{ orden.presupuesto?.numero_folio }} - {{ orden.vehiculo?.patente }}</p>
      </div>

      <div class="p-6">
        <label class="block text-sm font-medium mb-2">Seleccione un técnico</label>
        <select v-model="empleadoSeleccionado" class="w-full rounded-lg px-3 py-2.5 neutro-font neutro-font font-bold">
      ...
      <div class="px-6 py-4 flex justify-end gap-3">
        <button @click="modalAsignar = false" class="px-4 py-2 text-sm font-medium neutro-font neutro-font rounded-lg">
          Cancelar
        </button>
        <button 
          @click="asignarOrden" 
          :disabled="!empleadoSeleccionado || cargando"
          class="px-4 py-2 text-s neutro-font neutro-font rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
        >
