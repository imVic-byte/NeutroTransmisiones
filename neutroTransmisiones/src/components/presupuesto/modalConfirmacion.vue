...
<template>
  <div class="flex flex-col gap-4 w-full justify-center">
    <button v-if="showButtons && !editado" @click="confirm = true" class="neutro-primary px-4 py-2 rounded-lg text-white cursor-pointer">
      <p>Confirmar</p>
    </button>
    ...
    <button v-if="estado !== 1" @click="ejecutarBorrador" class="text-white px-4 py-2 rounded-lg cursor-pointer flex items-center gap-2 justify-center">
      <svg class="w-6 h-6 text-white dark:neutro-font" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
  ...
  <div v-if="confirm" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
    <div class="relative w-full max-w-md neutro-background rounded-lg shadow p-6 text-center">
      ...
      <div class="flex justify-center gap-3">
        ...
        <button @click="confirm = false" class="neutro-font px-4 py-2 rounded cursor-pointer hover:scale-105 transition-all">
          Cancelar
        </button>
      </div>
    </div>
