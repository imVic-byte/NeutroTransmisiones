...
<template>
  <div class="flex flex-col gap-4 justify-center">
    <button v-if="showButtons && !editado" @click="confirm = true" class="neutro-primary px-4 py-2 rounded-lg text-white cursor-pointer hover:scale-105 transition-all">
      <p>Confirmar</p>
    </button>
    ...
    <button v-if="editado" @click="ejecutarConfirmacionEdit" class="neutro-primary px-4 py-2 rounded-lg text-white cursor-pointer hover:scale-105 transition-all">
      <p>Confirmar</p>
    </button>
    ...
    <button @click="ejecutarPdf" class="neutro-font px-4 py-2 rounded-lg text-white cursor-pointer hover:scale-105 transition-all">
      PDF
    </button>
    <button v-if="ShowOt" @click="ejecutarIrAOT" class="neutro-font px-4 py-2 rounded-lg text-white cursor-pointer hover:scale-105 transition-all">
      Ir a OT 
    </button>
    <button v-if="ShowOt" @click="editarPresupuesto" class="neutro-font px-4 py-2 rounded-lg text-white cursor-pointer hover:scale-105 transition-all flex items-center gap-2 justify-center">
  ...
  <div v-if="confirm" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
    <div class="relative w-full max-w-md servi-adapt-bg rounded-lg shadow p-6 text-center">
      <h3 class="mb-5 text-lg font-normal neutro-font">¿Estás seguro de confirmar el presupuesto?</h3>
  ...
  <div v-if="discard" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
    <div class="relative w-full max-w-md servi-adapt-bg rounded-lg shadow p-6 text-center">
      <h3 class="mb-5 text-lg font-normal neutro-font">¿Estás seguro de descartar el presupuesto?</h3>
      <div class="flex justify-center gap-3">
