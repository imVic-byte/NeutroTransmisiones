    ...
    <div class="hidden md:block servi-adapt-bg rounded-xl shadow-sm overflow-hidden border border-gray-100">
        <table class="w-full text-left">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                >
                    <td class="p-4">
                        <span class="neutro-primary text-white px-2.5 py-1 rounded text-xs font-black tracking-wider uppercase shadow-sm">
                            {{ item.vehiculo?.patente || 'S/P' }}
                        </span>
                    </td>
                    <td class="p-4">
                        <div class="flex flex-col">
                            <span class="text-sm font-bold neutro-font">{{ item.vehiculo?.marca }} {{ item.vehiculo?.modelo }}</span>
                    ...
                    <td class="p-4">
                        <div v-if="item.cliente" class="flex flex-col">
                            <span class="text-sm font-semibold neutro-font">{{ item.cliente.nombre }} {{ item.cliente.apellido }}</span>
                    ...
                    <td class="p-4">
                        <p class="text-sm neutro-font truncate max-w-[200px]" :title="item.diagnostico">
        ...
        <div v-if="lista.length === 0" class="p-16 text-center">
            ...
            <p class="neutro-font text-xl font-bold">No hay vehículos en taller</p>
