...
<template>
  <div class="neutro-font neutro-primary p-2 rounded-xl mt-2 flex flex-col gap-2">
      <div v-if="fotosTomadas.length > 0" class="grid grid-cols-2 gap-3">
        <div v-for="(foto, index) in fotosTomadas" :key="index" class="relative group ">
          ...
          <img 
            :src="foto.url" 
            class="w-full h-32 object-cover rounded-lg border-2 neutro-font-border shadow-sm"
          />
    ...
    <button 
      @click="activarCamara"
      type="button"
      class="w-full neutro-font neutro-font text-white p-4 rounded-xl shadow-lg flex items-center justify-center gap-3 transition-transform transform active:scale-95"
    >
    ...
    <button @click="guardarFotos" :disabled="!tieneFotos" class="w-full text-lg font-bold neutro-font neutro-font text-white p-4 rounded-xl shadow-lg flex items-center justify-center gap-3 transition-transform transform active:scale-95">
      Guardar
    </button>
    <input 
      ref="inputCamara"
