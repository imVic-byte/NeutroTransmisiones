...
<template>
  <div class="neutro-background min-h-screen pb-15">
    <navbar titulo="NeutroTransmisiones" subtitulo="Chequeos" class="sticky top-0 z-50" />
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="flex justify-between items-center mb-6">
        <div>
          <h2 class="text-xl font-bold neutro-font">Listado de Chequeos</h2>
          <p class="text-sm neutro-font">Historial de chequeos completos realizados</p>
        </div>
        <button @click="abrirModal"
          class="neutro-primary cursor-pointer rounded-full text-white font-bold py-2 px-4 shadow-sm border border-gray-100 hover:opacity-80 transition-all flex items-center gap-2 active:scale-95">
      ...
      <div class="neutro-background rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-300 max-h-[80vh] flex flex-col">
        ...
        <div class="overflow-y-auto flex-1 divide-y divide-gray-100">
          <div v-for="ot in misOTs" :key="ot.id" @click="seleccionarOT(ot)"
            ...
            <!-- Info del vehículo -->
            <div class="flex items-center justify-between mb-2">
              <span class="font-bold neutro-font text-base">OT #{{ ot.id }}</span>
              <span class="px-2 py-1 bg-yellow-100 text-yellow-800 font-bold rounded text-xs">
                {{ ot.vehiculo?.patente || 'S/P' }}
              </span>
            </div>
            <div class="flex items-center gap-2 mb-2">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 neutro-font opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              ...
              <span class="text-sm neutro-font">{{ ot.vehiculo?.marca }} {{ ot.vehiculo?.modelo }} {{ ot.vehiculo?.anio || '' }}</span>
              <span v-if="ot.vehiculo?.color" class="text-xs neutro-font opacity-60">· {{ ot.vehiculo.color }}</span>
            </div>
            ...
            <div class="flex items-center gap-2">
              ...
              <span v-if="ot.ficha_de_trabajo?.cliente?.telefono" class="text-xs neutro-font opacity-60">
                · +56 {{ ot.ficha_de_trabajo.cliente.telefono }}
              </span>
            </div>

            <p v-if="ot.diagnostico" class="text-xs neutro-font mt-2 truncate opacity-70">{{ ot.diagnostico }}</p>
          </div>

          <!-- Sin OTs -->
          <div v-if="misOTs.length === 0" class="p-8 text-center">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 mx-auto neutro-font opacity-30 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
            <p class="neutro-font font-medium">No tienes OTs asignadas</p>
            <p class="text-sm neutro-font opacity-60 mt-1">Debes tener OTs activas para realizar un chequeo.</p>
          </div>
        </div>
        ...
        <div class="p-4 border-t border-gray-100 shrink-0">
          <button @click="mostrarModal = false"
            class="w-full py-2.5 rounded-xl font-bold neutro-font neutro-font transition-transform active:scale-95 cursor-pointer">
            Cancelar
          </button>
        </div>
      </div>
