...
<template>
  <div class="neutro-background min-h-screen pb-15">
    <navbar :titulo="'NeutroTransmisiones'" subtitulo="Vehículos Estacionados" search-input="false" class="sticky top-0 z-50"/>
    
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
<volver />
      <div class="flex justify-between items-center mb-6">
        <div>
          <h2 class="text-xl font-bold neutro-font">Vehículos Estacionados</h2>
          <p class="text-sm neutro-font">{{ vehiculosEnTaller.length }} vehículo{{ vehiculosEnTaller.length !== 1 ? 's' : '' }} actualmente estacionados</p>
        </div>
      </div>
          ...
          <div class="p-4">
            ...
            <div class="flex justify-between items-center mb-3">
              <div class="flex items-center gap-2">
                <span class="text-xl font-black neutro-font tracking-wide">{{ item.vehiculo?.patente || 'S/P' }}</span>
                <span class="bg-gray-100 text-[10px] font-black neutro-font px-1.5 py-0.5 rounded">OT #{{ item.id }}</span>
              </div>
              <span 
            ...
            <!-- Info vehículo -->
            <div v-if="item.vehiculo?.marca || item.vehiculo?.modelo" class="text-sm font-bold neutro-font mb-3">
