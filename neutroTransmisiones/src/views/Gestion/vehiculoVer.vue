...
<template>
  <div class="neutro-background min-h-screen">
    <navbar class="navbar" titulo="NeutroTransmisiones" subtitulo="Detalle de Vehículo" />

    <div v-if="vehiculo" class="neutro-background min-h-screen pb-24">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 pt-4">

      <!-- Botón volver -->
      <button @click="router.push({ name: 'listado-vehiculos' })" class="mb-4 flex items-center gap-1 text-sm neutro-font hover:opacity-80 transition cursor-pointer">
          ...
          <div class="servi-adapt-bg rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="neutro-primary p-6 flex flex-col items-center relative">
              ...
              <div class="absolute top-4 right-4 flex gap-2">
                ...
                <template v-else>
                  <button @click="guardarVehiculo" class="p-2 neutro-font rounded-lg transition cursor-pointer hover:translate-y-1" title="Guardar">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="lg:w-2/3">
          <div class="servi-adapt-bg rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div v-if="vehiculo.cliente" class="p-6">
              ...
              >
                <div class="w-14 h-14 rounded-full neutro-primary flex items-center justify-center text-white text-lg font-bold shrink-0">
                  {{ ((vehiculo.cliente.nombre?.[0] || '') + (vehiculo.cliente.apellido?.[0] || '')).toUpperCase() }}
                </div>
                <div class="min-w-0 flex-1">
                  <p class="font-bold neutro-font">{{ camelCase(vehiculo.cliente.nombre) }} {{ camelCase(vehiculo.cliente.apellido) }}</p>
                  <div class="flex flex-col sm:flex-row sm:gap-4 mt-1">
                    <p v-if="vehiculo.cliente.email" class="text-sm neutro-font truncate">
                    ...
                    <p v-if="vehiculo.cliente.telefono" class="text-sm neutro-font">
                      <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 inline mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 neutro-font shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <div v-else class="p-10 text-center">
              <p class="neutro-font font-medium">Sin propietario asignado</p>
            </div>
          </div>
        </div>
