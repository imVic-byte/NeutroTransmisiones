...
<template>
  <div class="neutro-background min-h-screen">
    <navbar class="navbar" titulo="NeutroTransmisiones" subtitulo="Clientes" />
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 pt-4">
      ...
      <div class="mb-5">
        <div class="relative">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 absolute left-3 top-1/2 -translate-y-1/2 neutro-font" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          <input
            ...
            class="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-100 servi-adapt-bg neutro-font text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent shadow-sm"
          />
        </div>
      </div>
      ...
      <div class="hidden md:block servi-adapt-bg rounded-xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <tr v-for="cliente in clientesFiltrados" :key="cliente.id" class="hover:opacity-80 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <p class="font-semibold neutro-font">{{ camelCase(cliente.nombre) }} {{ camelCase(cliente.apellido) }}</p>
                    </div>
                  </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap neutro-font">{{ cliente.email || '—' }}</td>
                <td class="px-6 py-4 whitespace-nowrap neutro-font">
                  <span v-if="cliente.telefono">+{{ cliente.codigo_pais || '56' }} {{ cliente.telefono }}</span>
                  <span v-else>—</span>
                </td>
                <td class="px-6 py-4 whitespace-nowrap neutro-font">
                  <button @click="verCliente(cliente.id)" class="neutro-primary neutro-font px-2 py-1 rounded-full hover:opacity-80 transition-colors">
                    <svg class="w-6 h-6 neutro-font" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
        ...
        <div v-if="clientesFiltrados.length === 0" class="p-10 text-center">
          <p class="neutro-font font-medium">{{ busqueda ? 'No se encontraron resultados' : 'No hay clientes registrados' }}</p>
        </div>
      </div>
      ...
      <div class="md:hidden space-y-3">
        ...
        >
          <div class="flex items-center gap-3 mb-3">
            ...
            <div class="min-w-0">
              <p class="font-bold neutro-font text-sm truncate">{{ camelCase(cliente.nombre) }} {{ camelCase(cliente.apellido) }}</p>
            </div>
          </div>
          <div class="space-y-1.5 text-sm border-b border-gray-100 pb-2">
            <div class="flex items-center gap-2 neutro-font">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 neutro-font shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
          ...
          <button @click="verCliente(cliente.id)" class="neutro-primary mt-2 w-full neutro-font px-2 py-1 rounded-xl hover:opacity-80 transition-colors">
            Ver Cliente
          </button>
        </div>
