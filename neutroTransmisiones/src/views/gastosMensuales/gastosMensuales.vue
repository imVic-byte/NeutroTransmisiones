...
<template>
  <div class="min-h-screen neutro-background font-sans">
    <navbar titulo="NeutroTransmisiones" :subtitulo="'Gastos de la Empresa'" class="navbar"></navbar>
    <div class="max-w-6xl mx-auto space-y-6 pt-5 pb-30 px-4 md:px-8">
      ...
      <div :class="mostrarStats ? 'block' : 'hidden md:grid'" class="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
        <div class="neutro-primary p-4 md:p-5 rounded-lg shadow-lg border-b-4 border-yellow-500 relative overflow-hidden">
        ...
        <div class="neutro-primary p-4 md:p-5 rounded-lg shadow-lg border-b-4 border-slate-300">
      ...
      <template v-else>
        <div class="grid grid-cols-1 gap-3 md:hidden mt-4">
          ...
          <div v-for="ficha in fichasGastos" :key="ficha.id" class="neutro-primary p-4 rounded-lg shadow-lg">
            ...
            <button @click="handleRedirigir(ficha.id)" class="w-full neutro-font py-2.5 rounded-md font-semibold text-sm">
              Ver Ficha
            </button>
          </div>
        </div>
          ...
          <table class="min-w-full">
            ...
            <tbody>
              ...
              <tr v-for="ficha in fichasGastos" :key="ficha.id" class="hover:bg-slate-50 transition-colors">
                <td class="px-6 py-4 whitespace-nowrap font-medium neutro-font capitalize">{{ formatearMes(ficha.fecha) }}</td>
                <td class="px-6 py-4 whitespace-nowrap font-bold neutro-font">{{ formatearDinero(ficha.total_general) }}</td>
                ...
                <td class="px-6 py-4 whitespace-nowrap flex neutro-font justify-center">
