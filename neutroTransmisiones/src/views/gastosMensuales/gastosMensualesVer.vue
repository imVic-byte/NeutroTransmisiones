...
import { formatearFecha, formatearDinero, formatearMes } from '@/js/formateadores.js';
...
<template>
  <div class="min-h-screen neutro-background font-sans">
    <navbar titulo="NeutroTransmisiones" :subtitulo="'Detalle de Gastos'" class="navbar"></navbar>
    <div class="max-w-7xl mx-auto pt-5 pb-30 px-4 md:px-8">
      ...
      <div v-if="cargando" class="flex flex-col items-center justify-center py-20">
        ...
        <p class="neutro-font font-bold">Cargando detalles de la ficha...</p>
      </div>
      ...
      <template v-else-if="ficha">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mt-4 mb-6">
          <div>
            ...
            <p class="text-sm neutro-font pl-4">ID de Registro: #{{ ficha.id }}</p>
          </div>
          ...
          <div class="flex gap-2 w-full md:w-auto">
            ...
            <button v-if="ficha.estado === 1" @click="irAEditar"
              class="flex-1 md:flex-none neutro-font text-slate-900 px-5 py-2 rounded shadow-md font-semibold text-sm">
              Editar Ficha
            </button>
            ...
            <div class="servi-adapt-bg rounded-lg shadow-sm border border-gray-100">
                ...
                <div v-for="gasto in detalles" :key="gasto.id" class="p-3 rounded-lg border border-gray-100">
                  <div class="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p class="text-xs neutro-font mt-0.5">{{ formatearFecha(gasto.fecha_emision) }}</p>
                    </div>
                    <span
              ...
              <div v-if="detalles.length > 0" class="hidden md:block overflow-x-auto">
                <table class="min-w-full">
                  <thead>
                    <tr>
                      <th class="px-6 py-3 text-left text-xs font-bold neutro-font uppercase">Concepto
                      </th>
                      <th class="px-6 py-3 text-left text-xs font-bold neutro-font uppercase">Fecha</th>
                      <th class="px-6 py-3 text-left text-xs font-bold neutro-font uppercase">Categoría
                      </th>
                      <th class="px-6 py-3 text-center text-xs font-bold neutro-font uppercase">Adjunto
                      </th>
                      <th class="px-6 py-3 text-right text-xs font-bold neutro-font uppercase">Monto
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="gasto in detalles" :key="gasto.id" class="border-t border-gray-100 hover:bg-slate-50">
                      <td class="px-6 py-4 text-sm neutro-font font-medium">{{ gasto.descripcion }}</td>
                      <td class="px-6 py-4 whitespace-nowrap text-sm neutro-font">{{
                        formatearFecha(gasto.fecha_emision) }}</td>
            ...
            <div class="servi-adapt-bg rounded-xl shadow-lg border-t-4 border-yellow-500 p-4 md:p-6 lg:sticky lg:top-6">
              ...
              <div class="space-y-3 md:space-y-4 mb-6 md:mb-8">
                <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                  ...
                  <span class="font-bold neutro-font">{{ formatearDinero(ficha.total_administrativo) }}</span>
                </div>
                <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                  <span class="neutro-font text-sm font-medium">Total Operacional</span>
                  <span class="font-bold neutro-font">{{ formatearDinero(ficha.total_operacional) }}</span>
                </div>
              </div>
    ...
    <div v-if="mostrarConfirmacionCierre" class="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black/60 backdrop-blur-sm">
      <div class="neutro-background rounded-xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        <div class="p-6 space-y-3">
          <p class="neutro-font font-medium">¿Estás seguro de que deseas cerrar este mes?</p>
