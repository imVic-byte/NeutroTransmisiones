...
import { formatearFecha, formatearMes } from '@/js/formateadores.js';
  ...
  >
    ...
    <div class="flex justify-between border-b-4 border-[#1f3d64] pb-4 mb-6">
      <div class="flex items-center gap-4">
        <span class="w-24 h-24 rounded-full overflow-hidden border border-[#e5e7eb]">
            <img class="w-full h-full object-cover" src="../../img/Logo.jpg" alt="Logo NeutroTransmisiones">
        </span>
        <div>
    ...
    <div class="mb-8">
      ...
      <ul class="text-[#374151] space-y-1 flex flex-wrap gap-x-8 gap-y-2">
        <li><span class="font-bold text-[#111827]">Razón Social:</span> {{ datosEmpresa.razon_social || 'NeutroTransmisiones' }}</li>
    ...
    <div class="mb-8 border border-[#e5e7eb] rounded-lg overflow-hidden">
      <table class="w-full text-left border-collapse">
        ...
        <tbody class="text-[#1f2937] text-[11px]">
          ...
          >
            <td class="p-3">{{ formatearFecha(item.fecha_emision) }}</td>
            ...
            <td class="p-3 text-right font-bold">{{ formatearDinero(item.monto) }}</td>
          </tr>
    ...
    <div class="absolute bottom-10 left-0 right-0 text-center border-t border-[#e5e7eb] pt-4 mx-10">
      <p class="text-[#9ca3af] text-[9px] uppercase tracking-wide">
        Documento Generado Internamente - NeutroTransmisiones
      </p>
    </div>
