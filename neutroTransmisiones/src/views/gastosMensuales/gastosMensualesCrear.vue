...
<template>
  <div class="min-h-screen neutro-background font-sans">
    <navbar titulo="NeutroTransmisiones" :subtitulo="'Crear ficha de gastos fijos'" class="navbar"></navbar>
    <div class="max-w-7xl mx-auto pt-5 pb-30 px-4 md:px-8">
      ...
      <div class="mb-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mt-2">
          <h1 class="text-xl md:text-3xl font-bold neutro-font border-l-4 border-yellow-500 pl-3 capitalize">
            Ficha de Gastos: {{ mesFichaMostrado }}
          </h1>
          <div class="flex gap-2 w-full md:w-auto">
            <button @click="cerrarMes" :disabled="cargando" class="flex-1 md:flex-none neutro-primary text-white px-5 py-2 rounded shadow-md font-semibold text-sm disabled:opacity-50">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="neutro-secondary rounded-lg shadow-sm overflow-hidden border border-gray-100">
            <div class="neutro-primary px-6 py-4 flex justify-between items-center border-yellow-500 border-b-4">
            ...
            <form @submit.prevent="agregarGasto" class="p-6 space-y-4">
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="flex flex-col">
                  ...
                  <input v-model="concepto" type="text" required class="w-full neutro-font neutro-secondary border border-gray-100 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium text-sm" placeholder="Ej. Pago de internet">
                </div>
                
                <div class="flex flex-col">
                  <label class="text-xs font-bold neutro-font uppercase tracking-wider mb-1">Monto *</label>
                  <input v-model="monto" type="number" required min="1" class="w-full neutro-font neutro-secondary border border-gray-100 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium text-sm" placeholder="0">
                </div>
                
                <div class="flex flex-col">
                  <label class="text-xs font-bold neutro-font uppercase tracking-wider mb-1">Categoría *</label>
                  <select v-model="categoria" class="w-full neutro-font neutro-secondary border border-gray-100 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium text-sm">
                ...
                <div class="flex flex-col">
                  ...
                  <input v-model="fechaEmision" type="date" required class="w-full neutro-font neutro-secondary border border-gray-100 rounded-lg p-2.5 focus:ring-blue-500 focus:border-blue-500 font-medium text-sm">
                </div>

                <div class="col-span-1 md:col-span-2 flex flex-col mt-2">
                  <label class="text-xs font-bold neutro-font uppercase tracking-wider mb-2">Comprobante (Opcional)</label>
          ...
          <div class="neutro-secondary rounded-lg shadow-sm border border-gray-100">
              ...
              <div v-for="gasto in listaGastos" :key="gasto.id_temporal" class="p-3 rounded-lg border border-gray-100">
                <div class="flex justify-between items-start mb-2">
                  <div class="flex gap-3 items-center">
                    ...
                    <div>
                      <p class="font-bold neutro-font text-sm">{{ gasto.descripcion }}</p>
                      <p class="text-xs neutro-font mt-0.5">{{ gasto.fecha_emision }}</p>
                    </div>
                  </div>
            ...
            <div v-if="listaGastos.length > 0" class="hidden md:block overflow-x-auto">
              <table class="min-w-full">
                <thead>
                  <tr>
                    <th class="px-6 py-3 text-left text-xs font-bold neutro-font uppercase">Concepto</th>
                    <th class="px-6 py-3 text-left text-xs font-bold neutro-font uppercase">Fecha</th>
                    <th class="px-6 py-3 text-left text-xs font-bold neutro-font uppercase">Categoría</th>
                    <th class="px-6 py-3 text-right text-xs font-bold neutro-font uppercase">Monto</th>
                    <th class="px-6 py-3 text-center text-xs font-bold neutro-font uppercase">Acción</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="gasto in listaGastos" :key="gasto.id_temporal" class="border-t border-gray-100">
                    <td class="px-6 py-4 text-sm neutro-font font-medium flex items-center gap-3">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm neutro-font">{{ gasto.fecha_emision }}</td>
        ...
        <div class="lg:col-span-1 order-first lg:order-none">
          <div class="neutro-secondary rounded-xl shadow-lg p-4 md:p-6 lg:sticky lg:top-6">
            <h2 class="border-yellow-500 border-b-4 text-lg md:text-xl font-bold mb-4 md:mb-6 neutro-font uppercase tracking-wide">Resumen del Mes</h2>
            
            <div class="space-y-3 md:space-y-4 mb-6 md:mb-8">
              <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                <span class="neutro-font text-sm font-medium">Total Administrativo</span>
                <span class="font-bold neutro-font">{{ formatearDinero(totalAdministrativo) }}</span>
              </div>
              <div class="flex justify-between items-center border-b border-gray-200 pb-2">
                <span class="neutro-font text-sm font-medium">Total Operacional</span>
                <span class="font-bold neutro-font">{{ formatearDinero(totalOperacional) }}</span>
              </div>
            </div>

            <div class="neutro-primary p-3 md:p-4 rounded-xl flex justify-between items-center mb-4 md:mb-6 shadow-md">
            ...
            <p class="text-xs neutro-font text-center mt-3 md:mt-4 opacity-70">Estado actual: Pendiente de Revisión</p>
          </div>
        </div>
