...
const obtenerTalleres = async () => {
  try {
    const { data, error } = await supabase.from('neutro_taller').select('*').order('id', { ascending: true })
...
const obtenerCapacidadMaximaTaller = async () => {
  const{data, error} = await supabase.from('neutro_taller').select('capacidad_maxima').eq('id', tallerSeleccionado.value).single()
  if (error) throw error
    ...
    <main class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-2 pb-20">
      <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between">
        ...
        <select v-model="tallerSeleccionado" @change="cambiarTaller" class="text-sm font-medium servi-adapt-bg neutro-font border border-gray-100 rounded-lg px-3 py-1.5 shadow-sm focus:ring-2 focus:ring-blue-200 focus:border-blue-400 outline-none cursor-pointer">
          ...
          -->
          <span v-if="!esFinDeSemana" class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-50 text-neutro-primary border border-blue-100">
      ...
      <div class="grid grid-cols-2 gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-4">
        <div @click="VehiculosEnTaller" class="neutro-primary overflow-hidden cursor-pointer rounded-xl shadow-sm border border-gray-100 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 active:scale-95">
        ...
        <div @click="SinAsignar" class="neutro-primary overflow-hidden cursor-pointer rounded-xl shadow-sm border border-gray-100 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 active:scale-95">
          <div class="p-4 sm:p-5">
            <div class="flex items-center">
              <div class="flex-shrink-0">
        ...
        <div @click="PresupuestosSemana" class="neutro-primary overflow-hidden cursor-pointer rounded-xl shadow-sm border border-gray-100 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 active:scale-95">
          <div class="p-4 sm:p-5">
            <div class="flex items-center">
              <div class="flex-shrink-0"> 
        ...
        <div @click="ListoParaEntregar" class="neutro-primary overflow-hidden cursor-pointer rounded-xl shadow-sm border border-gray-100 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 active:scale-95">
          <div class="p-4 sm:p-5">
            <div class="flex items-center">
              <div class="flex-shrink-0">
          ...
          <table class="w-full text-sm text-left">
            ...
            <tbody class="divide-y divide-gray-200">
              <tr v-for="ot in misOTs" :key="ot.id" @click="verOT(ot.id)" class="hover:opacity-80 transition-colors cursor-pointer">
                ...
                <td class="px-5 py-3.5 neutro-font truncate max-w-[200px]">{{ ot.diagnostico || 'Sin diagnóstico' }}</td>
                <td class="px-5 py-3.5">
