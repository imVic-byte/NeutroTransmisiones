...
const handleRedirigir = (id) => {
  router.push({ name: 'finanzasVer', params: { id } })
}

const handleRegistrar = () => {
  router.push({ name: 'finanzasCrear' })
}
</script>
  ...
  <div class="min-h-screen neutro-background font-sans">
    ...
    <div class="max-w-6xl mx-auto space-y-6 pt-5 pb-30 px-4 md:px-8">
      ...
      <div :class="mostrarStats ? 'block' : 'hidden md:block'">
        <div class="flex flex-col md:flex-row md:items-end gap-3 neutro-primary text-white p-4 rounded-lg shadow-sm">
      ...
      <div :class="mostrarStats ? 'block' : 'hidden md:grid'" class="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
        <div class="neutro-primary p-4 md:p-5 rounded-lg shadow-lg border-b-4 border-green-500">
        ...
        <div class="neutro-primary p-4 md:p-5 rounded-lg shadow-lg border-b-4 border-red-500">
        ...
        <div class="neutro-primary p-4 md:p-5 rounded-lg shadow-lg border-b-4 border-yellow-500">
      ...
      <div v-if="cargando" class="flex justify-center items-center py-10">
        <p class="neutro-font font-bold">Cargando registros...</p>
      </div>
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-1 gap-3 md:hidden mt-4">
          ...
          <div v-for="t in transaccionesPaginadas" :key="t.id" class="neutro-primary p-4 rounded-lg shadow-lg">
            ...
            <button @click="handleRedirigir(t.id)" class="w-full neutro-primary text-white py-2.5 rounded-md font-semibold text-sm">
              Ver Detalle
            </button>
          </div>
        </div>
            ...
            <table class="min-w-[800px] w-full">
              ...
              <tbody class="neutro-secondary">
                <tr v-if="transacciones.length === 0">
                  <td colspan="6" class="px-6 py-4 text-center text-white font-medium">No hay transacciones disponibles.</td>
                </tr>
                
                <tr @click="handleRedirigir(t.id)" v-for="t in transaccionesPaginadas" :key="t.id" class=" transition-colors">
                  <td class="px-4 py-4 whitespace-nowrap text-sm font-medium text-white">{{ formatearFecha(t.fecha) }}</td>
                  <td class="px-4 py-4 text-sm text-white max-w-xs truncate" :title="t.descripcion">{{ t.descripcion }}</td>
                  <td class="px-4 py-4 text-sm text-white max-w-xs truncate" :title="t.proveedor">{{ t.proveedor }}</td>
                  ...
                  <td class="px-4 py-4 whitespace-nowrap text-right font-bold text-white">{{ formatearDinero(t.valor_iva_incluido) }}</td>
                  <td class="px-4 py-4 whitespace-nowrap flex text-white justify-center">
