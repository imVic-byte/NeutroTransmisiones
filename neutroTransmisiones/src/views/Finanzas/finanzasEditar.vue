...
const cerrarModal = () => {
  const exito = modalState.value.exito
...
<template>
  <div class="min-h-screen neutro-background font-sans">
    <navbar titulo="NeutroTransmisiones" subtitulo="Editar Transacción" class="navbar"></navbar>
    
    <div class="max-w-4xl mx-auto space-y-6 pt-5 pb-30 px-4 md:px-8">
      <div class="flex justify-between">
        <volver></volver>
        <div class="flex gap-2">
          <button v-if="transaccion.id_ficha_gastos" @click="irAFicha()" class="neutro-secondary text-white p-2 rounded-xl font-bold text-sm">ir a Ficha</button>
          <button v-if="transaccion.id_deuda" @click="irADeuda()" class="neutro-secondary text-white p-2 rounded-xl font-bold text-sm">ir a Deuda</button>
        </div>
      </div>

      <template v-if="transaccion">
      <div class="neutro-secondary rounded-lg shadow-xl pb-10">
        <div class="flex neutro-primary rounded-t-lg border-b-4 border-yellow-500 flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4 p-6 servi-blue text-white">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 px-6">
          <div>
            ...
            <input type="date" v-model="transaccion.fecha" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 font-medium" />
          </div>
          <div>
            <label class="text-xs text-white font-bold uppercase tracking-wider mb-1 block">Proveedor / Cliente</label>
            <input type="text" v-model="transaccion.proveedor" placeholder="No especificado" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 font-medium" />
          </div>
          <div class="md:col-span-2">
            <label class="text-xs text-white font-bold uppercase tracking-wider mb-1 block">Descripción</label>
            <textarea v-model="transaccion.descripcion" rows="2" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 font-medium resize-none"></textarea>
          </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 px-6">
          <div>
            ...
            <input type="text" v-model="transaccion.documento" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 font-medium" />
          </div>
          <div>
            <label class="text-xs text-white font-bold uppercase tracking-wider mb-1 block">Forma de Pago</label>
            <input type="text" v-model="transaccion.forma_pago" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 font-medium" />
          </div>
          <div>
            <label class="text-xs text-white font-bold uppercase tracking-wider mb-1 block">Cantidad</label>
            <input type="number" v-model="transaccion.cantidad" min="1" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 font-medium" />
          </div>
        </div>

        <div class="mb-8 px-6">
          <label class="text-xs text-white font-bold uppercase tracking-wider mb-1 block">Observaciones</label>
          <textarea v-model="transaccion.observacion" rows="3" placeholder="Sin observaciones" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 font-medium resize-none"></textarea>
        </div>
        ...
        <div v-if="transaccion.transacciones_detalle && transaccion.transacciones_detalle.length > 0" class="mb-8 px-6">
          ...
          <div class="flex flex-col gap-2">
            <a v-for="(url, index) in transaccion.transacciones_detalle" :key="index" :href="url.url" target="_blank" rel="noopener noreferrer" class="text-blue-300 hover:text-blue-800 underline font-medium text-sm flex items-center gap-1">
      ...
      <div class="shadow-xl pb-2 rounded-lg neutro-secondary">
        <h3 class="font-bold text-lg servi-blue text-white neutro-primary p-4 mb-4 rounded-t-lg border-b-4 border-yellow-500">Desglose Financiero</h3>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8 px-6">
          <div>
            ...
            <input type="number" v-model="transaccion.valor_neto" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 font-medium" />
          </div>
          <div>
            <label class="text-xs text-white font-bold uppercase tracking-wider mb-1 block">IVA</label>
            <input type="number" v-model="transaccion.iva" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 font-medium" />
          </div>
          <div>
            <label class="text-xs text-white font-bold uppercase tracking-wider mb-1 block">Total</label>
            <input type="number" v-model="transaccion.valor_iva_incluido" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 text-blue-600 font-extrabold" />
          </div>
          <div>
            <label class="text-xs text-white font-bold uppercase tracking-wider mb-1 block">Precio Unitario</label>
            <input type="number" v-model="transaccion.precio_costo_unitario" class="w-full text-white border border-gray-200 servi-grey-font rounded-lg p-2.5 font-medium" />
          </div>
        </div>
      </div>
      ...
      <div class="flex justify-end gap-3 pb-10">
        ...
        <button @click="guardarCambios" class="px-6 py-3 rounded-lg neutro-secondary text-white font-bold hover:opacity-90 transition-all shadow-md">
          Guardar Cambios
        </button>
      </div>
