...
<template>
<div v-if="presupuesto" class="neutro-background min-h-screen">
            ...
            <div class="lg:w-2/3 space-y-6">
                <div class="servi-adapt-bg rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="space-y-2 text-sm neutro-font">
                                <p class="flex flex-col">
                                    <span class="font-bold neutro-font">{{ camelCase(presupuesto.cliente?.nombre) + ' ' + camelCase(presupuesto.cliente?.apellido) || 'No registrado' }}</span>
                                    <span class="neutro-font text-xs">Cliente</span>
                                </p>
                                <p>
                                    <span class="block text-xs neutro-font">Email</span>
                                    {{ presupuesto.cliente?.email || 'No registrado' }}
                                </p>
                                <p>
                                    <span class="block text-xs neutro-font">Teléfono</span>
                        ...
                        <div>
                            <h3 class="text-xs font-semibold neutro-font uppercase tracking-wider mb-3">Datos del Vehículo</h3>
                            <div class="space-y-2 text-sm neutro-font">
                                <p class="flex flex-col">
                                    <span class="font-bold neutro-font">{{ presupuesto.vehiculo?.patente || 'S/P' }}</span>
                                    <span class="neutro-font text-xs">Patente</span>
                                </p>
                                <p>
                                    <span class="block text-xs neutro-font">Vehículo</span>
                                    {{presupuesto.vehiculo?.marca}} {{presupuesto.vehiculo?.modelo}} {{presupuesto.vehiculo?.anio}}
                                </p>
                                <p>
                                    <span class="block text-xs neutro-font">Diagnóstico Inicial</span>
                                    <span class="italic neutro-font">{{ camelCase(presupuesto.diagnostico) || 'No registrado' }}</span>
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="lg:w-1/3 space-y-6">
                <div v-if="presupuesto.detalle_presupuesto.length > 0" class="neutro-primary rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div class="space-y-3 servi-adapt-bg p-6 rounded-b-xl">
                        ...
                        <div class="flex justify-between items-center text-sm neutro-font">
                            <span>Neto</span>
                            <span class="font-medium">{{ formatarDinero(presupuesto.total_neto || 0) }}</span>
                        </div>
                        <div class="flex justify-between items-center text-sm neutro-font">
                            <span>IVA (19%)</span>
                            <span class="font-medium">{{ formatarDinero(presupuesto.iva || 0) }}</span>
                        </div>
                        ...
                        <div class="pt-4 mt-2 border-t border-gray-100 flex justify-between items-center">
                            ...
                            <span class="font-extrabold text-2xl neutro-font">{{ formatarDinero(presupuesto.total_final || 0) }}</span>
                        </div>
                    </div>
                </div>
