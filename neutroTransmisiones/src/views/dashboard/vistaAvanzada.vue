...
onMounted(() => {
  loading.value = true
  Promise.all([
    handleVehiculosEnTaller(),
    handleListosParaEntregar(),
    handleCotizaciones(),
    handleTrabajoReciente(),
    handleTraerEstados()
  ])
    .catch((error) => {
...
<template>
  <div>
    ...
    <main v-if="loading" class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-20 space-y-6">
    <!-- Cabecera Skeleton -->
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between animate-pulse">
      <div class="space-y-2">
        ...
        <div class="h-4 w-32 bg-white/5 rounded-lg"></div>
      </div>
      <div class="mt-4 sm:mt-0 h-8 w-36 bg-white/10 rounded-full"></div>
    </div>

    <!-- Acciones Rápidas Skeleton -->
    <div class="animate-pulse">
      <div class="h-6 w-36 bg-white/10 rounded-lg mb-3"></div>
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div v-for="i in 4" :key="i" class="h-12 bg-white/5 rounded-xl border border-white/5"></div>
      </div>
    </div>
    ...
    <div class="animate-pulse">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div v-for="i in 3" :key="i" class="h-28 bg-white/5 rounded-2xl border border-white/10"></div>
      </div>
    </div>

    <!-- Columnas Detalladas Skeleton -->
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-pulse">
      <div v-for="i in 2" :key="i" class="h-64 bg-white/5 rounded-2xl border border-white/10"></div>
    </div>
  </main>

  <!-- 2. Vista Real (Con Datos) -->
  <main v-else class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 pt-4 pb-20 space-y-6">
